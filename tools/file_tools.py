...
READ_FILE_SCHEMA = {
    "name": "read_file",
    "description": "Read a text file with line numbers and pagination. Use this instead of cat/head/tail in terminal. Output format: 'LINE_NUM|CONTENT'. Suggests similar filenames if not found. Use offset and limit for large files. Reads exceeding ~100K characters are truncated on a line boundary and return a next_offset; continue with offset to read the rest. Jupyter notebooks (.ipynb), Word documents (.docx), and Excel workbooks (.xlsx) are auto-extracted to readable text; PDF, legacy Office (.doc/.ppt/.xls), OpenDocument, RTF, and EPUB convert too when the optional anydoc converter is available (auto-installed on first use where installs are permitted). PDF conversion reads the text layer only: scanned/image pages yield no text, and when many pages come back empty the output ends with an EXTRACTION COVERAGE WARNING listing the affected pages — follow its instructions (render pages with pdftoppm and inspect via vision_analyze, or OCR) instead of treating the extraction as complete. NOTE: Cannot read images or other binary files — use vision_analyze for images. Use mode='sketch' to get a compact structural skeleton (signatures, classes, defs, imports) instead of full file content — saves tokens for large files when you only need the structure. For files longer than 200 lines, call with mode='sketch' FIRST to see the structure, then use mode='full' with offset to read only the section you need — this saves significant context budget.",
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "mode": {"type": "string", "enum": ["full", "sketch"], "description": "Read mode: 'full' (default) returns file content with line numbers. 'sketch' returns a compact structural skeleton — imports, class/function signatures, top-level constants — omitting bodies. For files > 200 lines, use 'sketch' first to see structure, then 'full' with offset to read specific sections.", "default": "full"}
        },
        "required": ["path"]
    }
}
...
def _read_file_sketch(path: str, task_id: str = "default") -> str:
    """Return a compact structural skeleton of a file — signatures, classes,
    imports, top-level constants — without function/method bodies.

    For .py files: uses the ``ast`` module for accurate extraction.
    For other text files: regex-based heuristics (class/def/function/struct
    lines, import lines, top-level constants).
    """
    ...
    try:
        content = _resolved.read_text(encoding="utf-8", errors="replace")
    except Exception as exc:
        ...
        try:
            ...
            for node in _ast.iter_child_nodes(tree):
                if isinstance(node, _ast.Import):
                    ...
                    skeleton_lines.append(f"L{node.lineno:>5} | import {names}")
                elif isinstance(node, _ast.ImportFrom):
                    names = ", ".join(a.name + (f" as {a.asname}" if a.asname else "") for a in node.names)
                    skeleton_lines.append(f"L{node.lineno:>5} | from {node.module or ''} import {names}")
                elif isinstance(node, (_ast.FunctionDef, _ast.AsyncFunctionDef)):
                    args = ", ".join(a.arg for a in node.args.args)
                    ret = _ast.unparse(node.returns) if node.returns else ""
                    ret_str = f" -> {ret}" if ret else ""
                    skeleton_lines.append(f"L{node.lineno:>5} | def {node.name}({args}){ret_str}:")
                elif isinstance(node, _ast.ClassDef):
                    bases = ", ".join(_ast.unparse(b) for b in node.bases)
                    bases_str = f"({bases})" if bases else ""
                    skeleton_lines.append(f"L{node.lineno:>5} | class {node.name}{bases_str}:")
                    for member in _ast.iter_child_nodes(node):
                        if isinstance(member, (_ast.FunctionDef, _ast.AsyncFunctionDef)):
                            args = ", ".join(a.arg for a in member.args.args)
                            ret = _ast.unparse(member.returns) if member.returns else ""
                            ret_str = f" -> {ret}" if ret else ""
                            skeleton_lines.append(f"L{member.lineno:>5} |     def {member.name}({args}){ret_str}:")
                        elif isinstance(member, _ast.Assign):
                            for t in member.targets:
                                if isinstance(t, _ast.Name):
                                    skeleton_lines.append(f"L{member.lineno:>5} |     {t.id} = ...")
                elif isinstance(node, _ast.Assign):
                    for t in node.targets:
                        if isinstance(t, _ast.Name):
                            skeleton_lines.append(f"L{node.lineno:>5} | {t.id} = ...")
        except SyntaxError:
            skeleton_lines.append("# (file has syntax errors — showing regex sketch)")
            ext = ".txt"  # fall through to regex

    if not skeleton_lines and ext != ".py":
        import re as _re
        for i, line in enumerate(content.splitlines(), 1):
            stripped = line.strip()
            if not stripped or stripped.startswith("#"):
                continue
            if _re.match(r'^(import |from .+ import )', stripped):
                skeleton_lines.append(f"L{i:>5} | {stripped}")
            elif _re.match(r'^(class |def |async def |function |struct |interface )', stripped):
                skeleton_lines.append(f"L{i:>5} | {stripped}")
            elif _re.match(r'^(const |let |var |static |final |public |private |protected )', stripped) and "=" in stripped:
                skeleton_lines.append(f"L{i:>5} | {stripped[:100]}")
    ...
    if not skeleton_lines:
        return _json.dumps({
            ...
            "sketch": True,
            "note": "No structural elements found. Use mode='full' to read the file.",
        }, ensure_ascii=False)
    ...
    result = {
        ...
        "hint": f"Showing {len(skeleton_lines)} structural lines out of {total_lines} total. Use mode='full' with offset to read specific sections.",
    }
...
def _handle_read_file(args, **kw):
    ...
    if mode == "sketch":
        return _read_file_sketch(path=args.get("path", ""), task_id=tid)

    # Auto-sketch: if mode is 'full' (default) and the file is large,
    # return the structural skeleton + a hint instead of dumping the
    # entire file into context. The model can then request specific
    # sections with offset. This saves significant tokens for large files.
    _path = args.get("path", "")
    _offset = args.get("offset", 1)
    if mode == "full" and _offset == 1 and _path:
        try:
            _resolved = _resolve_path_for_task(_path, tid)
            if _resolved.exists() and _resolved.suffix.lower() == ".py":
                _line_count = sum(1 for _ in _resolved.open(encoding="utf-8", errors="replace"))
                if _line_count > 500:
                    import json as _json
                    sketch_result = _read_file_sketch(_path, task_id=tid)
                    sketch_dict = _json.loads(sketch_result)
                    sketch_dict["auto_sketch"] = True
                    sketch_dict["hint"] = (
                        f"File has {_line_count} lines — showing structural skeleton instead of full content "
                        f"(auto-sketch for files > 500 lines). Use read_file(path='{_path}', mode='full', offset=N) "
                        f"to read a specific section, or mode='full' with offset=1 and limit=500 to force full read."
                    )
                    return _json.dumps(sketch_dict, ensure_ascii=False)
        except Exception:
            pass  # fall through to normal read

    return read_file_tool(path=_path, offset=_offset, limit=args.get("limit", 500), task_id=tid)
