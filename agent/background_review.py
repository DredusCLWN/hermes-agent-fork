...
_MEMORY_REVIEW_PROMPT = (
    ...
    "MEMORY HYGIENE (apply every pass, in order):\n"
    "  1. Source: only save facts the user stated directly or that came "
    "from tool output — NEVER model assumptions, guesses, or unverified "
    "inferences.\n"
    "  2. Dedup: if a fact you're about to add is already in memory "
    "(same meaning, different wording), use 'replace' — do NOT add a duplicate.\n"
    "  3. Contradiction: if a new fact contradicts an existing entry, "
    "use 'replace' — the old fact is wrong now.\n"
    "  4. Stale-refresh: scan entries for time-sensitive facts that "
    "may have changed. If the conversation revealed an update, 'replace'.\n"
    "  5. How-to: if a how-to entry already lives in a skill, remove it "
    "from memory (keep only a one-line pointer if needed). If no skill "
    "exists yet, keep the entry or convert it into a skill.\n\n"
