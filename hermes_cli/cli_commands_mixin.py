...
class CLICommandsMixin:
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        try:
            ...
            # Use platform-appropriate splitting: Windows backslashes are
            # literal; POSIX treats them as escapes. Avoid shell=True.
            try:
                editor_argv = shlex.split(editor, posix=(os.name != "nt"))
            except ValueError:
                # Malformed editor string (unclosed quote). Treat the whole
                # value as a single executable and still avoid the shell.
                editor_argv = [editor]
            try:
