...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    (memory providers via a plugin-dir scan). The module-level
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # Messaging-platform credentials were removed with the channel catalog.
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # TARGET profile's channel state, not the root install's. The gateway
    # status readers resolve process-level paths, so the profile directory
    # is passed explicitly for those (#71211).
    def _run():
        ...
        return {
            ...
            # Built-in messaging channel catalog removed; plugins may
            # register platforms later.
            "platforms": [],
            "gateway_state": runtime,
        }
...
async def update_messaging_platform(
    ...
    # No built-in messaging platforms remain (channel catalog removed);
    # plugin-registered platforms are managed through their own surface.
    raise HTTPException(
...
# post-selection model picker (`_configure_imagegen_model_for_plugin` in tools_config.py).
