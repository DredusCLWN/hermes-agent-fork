...
@_profile_scoped
def _(rid, params: dict) -> dict:
    return _err(rid, 5031, "pets feature removed")
...
@_profile_scoped
def _(rid, params: dict) -> dict:
    return _ok(rid, {"ok": False, "slug": str(params.get("slug") or "")})
...
def _(rid, params: dict) -> dict:
    ...
    rewind_count = int(meta.get("rewind_count", 0) or 0)
    cwd = meta.get("cwd", "") or ""
    ...
    lines.append(f"  Last activity:  {updated.strftime('%Y-%m-%d %H:%M')}")
    if cwd:
    ...
    lines.append(f"  Tokens spent:     {_fmt_tok(total_tok)}")
    if prompt_tok or completion_tok:
        lines.append(f"    ├─ Input:       {_fmt_tok(in_tok)}")
        if prompt_tok:
        ...
        if reasoning_tok:
            lines.append(f"    │  └─ Reasoning:  {_fmt_tok(reasoning_tok)}")
        lines.append(f"    ├─ Output:      {_fmt_tok(out_tok)}")
        if completion_tok:
            lines.append(f"    │  ├─ Completion: {_fmt_tok(completion_tok)}")
    if est_cost > 0:
        lines.append(f"    └─ Cost:        ${est_cost:.4f}")
    lines.append("")
    ...
    if compressions:
        ...
        if cumulative_saved > 0:
            lines.append(f"    ├─ Total saved:  ~{_fmt_tok(cumulative_saved)} tokens (cumulative)")
        if ineffective:
    ...
    if context_length:
        ...
        lines.append(f"  Context window:   {_fmt_tok(context_tokens)} / {_fmt_tok(context_length)}  ({ctx_pct:.0f}%)")
        lines.append(f"    └─ Headroom:    {_fmt_tok(headroom)} tokens available")
        lines.append("")
    ...
    try:
        if db is not None and key:
            msgs = db.get_messages(key) or []
            turns = sum(1 for m in msgs if m.get("role") == "user")
    except Exception:
        pass
    # Fallback: estimate turns from message_count
    if not turns and msg_count:
        turns = msg_count // 2
    ...
    try:
        ...
        if skills_dir.exists():
            skills_active = sum(1 for _ in skills_dir.glob("*.md"))
    ...
    if turns > 0 and (api_calls or db_api_calls) > 0:
        lines.append(f"  API calls/turn:  {(api_calls or db_api_calls) / turns:.1f}")
    ...
    # ── Health ──
    lines.append("── Health ───────────────────────────────────────────────────")
    lines.append(f"  Agent running:   {'Yes' if session.get('running') else 'No'}")
    if compressions:
        if ineffective > 2:
