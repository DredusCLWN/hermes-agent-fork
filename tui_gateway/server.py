...
# Activity — exactly what was missing when turns started
# exiting the gateway mid-output.
...
def _get_usage(agent) -> dict:
    g = lambda k, fb=None: (lambda v: v if isinstance(v, (int, float)) else 0)(getattr(agent, k, 0) or (getattr(agent, fb, 0) if fb else 0))
    usage = {
    ...
    _cleanup_raw = getattr(agent, "_cleanup_metrics", None) or {}
    _cleanup = _cleanup_raw if isinstance(_cleanup_raw, dict) else {}
