...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/DredusCLWN/hermes-agent-fork/${ref}/scripts/${scriptName}`;
  return new Promise((resolve, reject) => {
...
function resolveHermesHome() {
  ...
  if (IS_WINDOWS3 && process.env.LOCALAPPDATA) {
    ...
    const hasHermesData = (dir) => fileExists(path21.join(dir, "state.db")) || fileExists(path21.join(dir, "projects.db")) || fileExists(path21.join(dir, ".env"));
...
var PREVIEW_PDF_EXTENSIONS = /* @__PURE__ */ new Set([".pdf"]);
...
async function interceptSessionRequestForRemote(request2) {
  ...
  if (/^\/api\/sessions\/[^/]+(\/messages)?$/.test(pathname)) {
    ...
    if (globalRemoteActive()) {
      ...
      const path22 = `${pathname}?${passthroughParams.toString()}`;
      if (method === "GET") {
...
ipcMain.handle("hermes:selectSavePath", async (_event, options = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
